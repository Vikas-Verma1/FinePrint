      ...
      <div className="sc-row">
        {scenarios.map((sc, idx) => (
          <button key={idx} className={"sc-chip" + (idx === i ? " active" : "")} onClick={() => setI(idx)}>
            {sc.icon} <GlossaryText text={sc.name} /> · {sc.when}
          </button>
        ))}
      </div>
        ...
        {!s ? (
          ...
          <>
            <div className="sc-top">
              <span className="sc-name">{s.icon} <GlossaryText text={s.name} /></span>
            ...
            <div className="sc-why"><GlossaryText text={s.why} /></div>
            <div className="sc-clauses">{(s.clauses || []).map((c, k) => <span key={k} className="chip"><GlossaryText text={c} /></span>)}</div>
          </>
        )}
      </div>
    </>
