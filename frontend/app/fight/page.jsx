...
export default function FightPage() {
  ...
  function handleDenialFile(f) {
    setDenialFile(f);
    extractFileText(f).then((t) => setDenialExtracted(t && t.trim().length > 40 ? t : ""));
  }
  ...
  async function run() {
    ...
    try {
      // FASTER: send extracted text only when no file is attached
      const res = await appeal({
        denialFile,
        denialText: denialFile ? denialText : (denialText || denialExtracted),
      });
      await typing;
      if (progRef.current) progRef.current.style.width = "100%";
      const secs = Array.isArray(res?.sections) ? res.sections : [];
      if (!secs.length) throw new Error("Appeal generation returned an empty result.");
