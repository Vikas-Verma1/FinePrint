...
def _ocr(data: bytes) -> str:
    """OCR for scanned PDFs — needs poppler-utils + tesseract on the server."""
    try:
        from pdf2image import convert_from_bytes
        import pytesseract
        images = convert_from_bytes(data, dpi=150, first_page=1, last_page=10)
        return "\n".join(pytesseract.image_to_string(img) for img in images)
    except Exception as e:
        log.info("PDF OCR unavailable or failed: %s", e)
        return ""


def _ocr_image(data: bytes) -> str:
    """OCR for uploaded PNG/JPG images — needs tesseract on the server."""
    try:
        import io
        ...
        log.info("Image OCR unavailable or failed: %s", e)
        return ""
