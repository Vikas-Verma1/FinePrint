...
const MODEL = process.env.OPENAI_MODEL || (IS_GROQ ? "llama-3.3-70b-versatile" : "gpt-4o-mini");
...
export async function POST(request) {
  try {
    ...
    if (!message || message.length > 8000) return NextResponse.json({ error: "Invalid message" }, { status: 400 });

    const lang = body.lang === "hi" ? "hi" : "en";

    const conversation = Array.isArray(body.conversation)
      ? body.conversation.filter((m) => m && typeof m.content === "string" && (m.role === "user" || m.role === "assistant")).slice(-5)
      : [];

    const pc = body.policyContext && typeof body.policyContext === "object" ? body.policyContext : null;
    const hasPolicy = Boolean(pc && (pc.text || pc.reportJson));

    if (!OPENAI_API_KEY) return NextResponse.json({ error: "API key not configured" }, { status: 503 });

    let system = `You are FinePrint AI, the user's personal insurance-policy advocate.

GROUNDING RULES — FOLLOW ABSOLUTELY:
1. Answer policy questions ONLY from the user's document (<POLICY_DOCUMENT> / retrieved clauses / <ANALYSIS_REPORT>).
2. Cite the exact section ref when available (e.g. §4.2).
3. If the information is not in the document, say "Your uploaded policy does not mention <topic>." Never fill gaps with generic info.
4. If no document is provided and the user asks about "my policy", tell them to upload it on the Read page.
5. Educational questions may get a 1-2 sentence plain definition, then check the user's document.`;

    if (lang === "hi") system += `\n\nLANGUAGE RULE: Respond in Hindi (Devanagari). Keep terms like premium, sum insured, claim in English.`;

    // NEW: RAG retrieved clauses → clickable citations
    if (Array.isArray(body.retrieved) && body.retrieved.length) {
      system += `\n\nRETRIEVED CLAUSES FROM THE USER'S POLICY (cite their refs exactly like §4.2):\n` +
        body.retrieved.map((r) => `[${r.ref}] ${r.text}`).join("\n");
    }

    if (hasPolicy) {
      system += `\n\nThe user's policy file/name: ${String(pc.name || "Uploaded policy").replace(/"/g, "'")}`;
      if (pc.text) {
        const t = pc.text.length > MAX_POLICY_CHARS ? pc.text.slice(0, MAX_POLICY_CHARS) + "\n[…truncated…]" : pc.text;
        system += `\n\n<POLICY_DOCUMENT>\n${t}\n</POLICY_DOCUMENT>`;
      }
      if (pc.reportJson) system += `\n\n<ANALYSIS_REPORT>\n${pc.reportJson}\n</ANALYSIS_REPORT>`;
    }
...
//     // ===== NEW: Hindi mode =====
//     if (lang === "hi") {
//       system += `\n\nLANGUAGE RULE: Respond in Hindi (Devanagari script). Keep common insurance terms in English where natural (premium, sum insured, claim, policy, co-pay) but explain everything in simple, friendly Hindi.`;
//     }
